...
class ProductoSerializer(serializers.ModelSerializer):
    ...
    proveedor_nombre = serializers.CharField(
        source="proveedor_principal.razon_social", read_only=True
    )
    proveedor_ruc = serializers.CharField(
        source="proveedor_principal.ruc", read_only=True
    )
    ...
    def validate_isbn(self, value):
        if self.instance:
            if (
                Producto.objects.filter(isbn=value)
                .exclude(id=self.instance.id)
                .exists()
            ):
                raise serializers.ValidationError(
                    f"Ya existe un producto con el ISBN '{value}'"
                )
        else:
